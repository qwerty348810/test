...
?>
	...
	<div class="commentsList">
		...
		<?php else :
			if ( ! comments_open() ) {
				?>Комментарии закрыты.<?php
			} else {
				?><h5 id="comments-title">0 Комментариев</h5>
				Прокомментируй. Будь первым! <?php
			}
		?>
	...
	<div class="commentsForm">
		...
		<?php
		 ...
		 $defaults = array( 'fields' => apply_filters( 'comment_form_default_fields', array(
			'author' => '<input id="author" name="author" type="text" value="Имя*" data-value="Name*." />',
			'email'  => '<input id="email" name="email" type="text" value="Email*" data-value="Email*" />',
			'url'    => '<input id="url" name="url" type="text" value="Веб-сайт" data-value="Website" />' ) ),
			'comment_field' => '<textarea id="comment" name="comment" cols="83" rows="4" data-value="Message*">Сообщение*</textarea>',
