...
function foodrecipes_custom_breadcrumbs() {
  ...
  $foodrecipes_home = 'Главная'; // text for the 'Home' link
...
if(get_comments_number() > 0 ) {
	...
	if ( $foodrecipes_tag_list ) {
		$foodrecipes_utility_text = __( '<div class="post-category"> Категория : %1$s  - %3$s </div><div class="post-author"> от : %4$s </div> <div class="post-comment"> Комментарии: '.get_comments_number().'</div>', 'foodrecipes' );
	} elseif ( $foodrecipes_category_list ) {
		$foodrecipes_utility_text = __( '<div class="post-category"> Категория : %1$s  - %3$s </div><div class="post-author"> от : %4$s </div> <div class="post-comment"> Комментарии: '.get_comments_number().'</div>', 'foodrecipes' );
	} else {
		$foodrecipes_utility_text = __( '<div class="post-category"> Категория : %3$s </div><div class="post-author"> от : %4$s </div> <div class="post-comment"> Комментарии: '.get_comments_number().'</div>', 'foodrecipes' );
	}
} else {
	
	if ( $foodrecipes_tag_list ) {
		$foodrecipes_utility_text = __( '<div class="post-category"> Категория : %1$s  - %3$s </div><div class="post-author"> от : %4$s </div>', 'foodrecipes' );
	} elseif ( $foodrecipes_category_list ) {
		$foodrecipes_utility_text = __( '<div class="post-category"> Категория : %1$s  - %3$s </div><div class="post-author"> от : %4$s </div>', 'foodrecipes' );
	} else {
		$foodrecipes_utility_text = __( '<div class="post-category"> Категория : %3$s </div><div class="post-author"> от : %4$s </div>', 'foodrecipes' );
	}
}
...
return ' <br /><a class="more" href="'. get_permalink( get_the_ID() ) . '">Подробнее... </a>';
 }
