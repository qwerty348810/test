...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?> itemscope itemtype="http://schema.org/Recipe">
	...
	<header class="entry-header">
		...
		<div class="entry-meta">
			...
			<span itemprop="recipeType"> <?php echo get_the_term_list( $post->ID, 'type', '<b>Рубрика:</b> ', ', ' ); ?> </span>
			<span itemprop="recipeCuisine"> <?php echo get_the_term_list( $post->ID, 'cuisine', '<b>Кухня:</b> ', ', ' ); ?></span>
			...
			?> 
			<span itemprop="totalTime"> <?php 
					echo _e('<b>Время приготовления:</b> ','fabthemes'); 
					if( $duration['hours'] ){ echo ( $duration['hours'] ); echo _e('h ','fabthemes');} 
					if( $duration['minutes'] ){ echo ( $duration['minutes'] ); echo _e('m','fabthemes');} 
					
					?> 
			</span>
			<span> <?php echo _e('<b>Распечатать:</b> ','fabthemes');  echo get_post_meta( $post->ID, '_cmb_recipe_serving', true ); ?></span>	
			<span> 	<a href="javascript:window.print()"><i class="fa fa-print"></i>  Печать</a></span>
		</div><!-- .entry-meta -->
